...
import { getMyLeavesApi, cancelLeaveApi, getLeaveBalanceApi } from '../../api/leaves';
...
const MyLeaves = () => {
  ...
  const totalEntitlement = balanceData?.reduce((s, b) => s + b.cap_days,  0) || 0;
      ...
      <div className="mx-auto p-6 max-w-4xl">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-gray-400 text-sm">Track and manage your time-off requests.</p>
          </div>
          <Link
            to="/leave-request"
        ...
        <div className="gap-4 grid grid-cols-2 sm:grid-cols-4 mb-6">
          {[
            { label: 'Total Entitlement', value: `${totalEntitlement} Days`, accent: 'border-primary' },
            { label: 'Days Taken',         value: `${totalUsed} Days`,        accent: 'border-emerald-400' },
            { label: 'Pending Approval',   value: `${totalPending} Days`,     accent: 'border-amber-400' },
            { label: 'Remaining Balance',  value: `${totalRemaining} Days`,   accent: 'border-violet-400' },
        ...
        <div className="overflow-hidden card">
          <div className="flex justify-between items-center px-5 py-4 border-gray-100 border-b">
            <h2 className="font-semibold text-gray-800 text-sm">Recent Leave History</h2>
          ...
          {isLoading ? (
            ...
            <div className="py-12 text-gray-400 text-sm text-center">No leave requests yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-gray-100 border-b">
                  {['Leave Type', 'Duration', 'Days', 'Status', 'Actions'].map((h) => (
                    <th key={h} className="px-5 py-3 font-semibold text-gray-400 text-xs text-left uppercase tracking-wide">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {leaves.map((l) => {
                  ...
                  return (
                    <tr key={l._id} className="hover:bg-gray-50 border-gray-50 border-b transition-colors">
                      <td className="px-5 py-4">
                        <p className="font-medium text-gray-800">{l.leave_type_id?.name}</p>
                        {l.reason && <p className="max-w-[160px] text-gray-400 text-xs truncate">{l.reason}</p>}
                      </td>
                      ...
                      <td className="px-5 py-4">
                        ...
                        {l.admin_comment && (
                          <p className="mt-1 max-w-[140px] text-[11px] text-gray-400 truncate">
                            "{l.admin_comment}"
                          </p>
                        )}
                      </td>
